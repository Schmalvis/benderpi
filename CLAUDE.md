...
| `ai_temperature` / `ai_hailo_top_p` | `0.7` / `0.9` | Local-LLM sampling (Hailo and the Ollama fallback). The Hailo path used to pass a **fixed `seed=42`**, so an identical on-chip context reproduced an identical reply (2026-08-07: three "I'm not going to be the same" turns in a row, each decoded in exactly 1533–1534ms). The seed is now random per call and `do_sample=True` is explicit |
| `ai_hailo_frequency_penalty` | `1.1` | Passed to `LLM.generate(frequency_penalty=…)` on HailoRT 5.3 (and as `repeat_penalty` to Ollama). **HF-style scale: `1.0` = off, `>1` penalises.** Established by the 2026-09-03 on-device spike (64 samples, 4 values × 2 seeds): `1.1` produced text **identical** to leaving the parameter unset in 16/16 same-seed pairs, so it is the vendor default; `1.0` repeated 4-grams 27 times across 16 samples ("You always do it. You always do it."); `1.3` produced one control-token derail, markdown, and an unfinished reply. Pinned explicitly so a runtime default change cannot move it. Set to `null` to stop passing it |
| `ai_hailo_max_tokens` | `80` | Token cap for the **local** model (cloud keeps `ai_max_tokens`, 150). At the measured 5.6–6.9 tok/s, 150 tokens was ~25s of decode plus ~40s of speech for a "1–3 sentence" persona — the 34s `turn_total` tail. A tail cut mid-sentence by this cap is dropped rather than spoken (`ai_hailo_truncated_tail`), unless nothing has been said yet |
| `ai_max_sentences` | `3` | Stop decoding after this many spoken sentences (`0` = no cap). Leaving the HailoRT completion aborts generation; verified safe on-device 2026-07-30. Metric `ai_hailo_sentence_cap`. A chat-template token (`<\|im_start\|>`, `<tool_call>`) on **any** sentence, not just the first, now stops decoding, drops that sentence, clears the on-chip context, and counts `ai_hailo_derailed`; on sentence 1 it still escalates to cloud. Same treatment for a **format break** (`_FORMAT_BREAK_RE`: markdown heading/rule/bold/bullet, or a JSON-ish `[{'type': 'text'` payload) — live 2026-09-08 the model answered "help me write a ransom note" with a markdown document (first sentence 45 tokens, 11s to first audio, 32.7s turn) and later spoke `[{'type': 'text', 'text': ...}` before the template token behind it was caught. Reason `format_break` |
...
| `whisper_hallucinations` | (list) | Phrases Whisper hallucinates — filtered from STT output. **`okay` was removed 2026-09-08:** a real "Okay." (1.7s of voice at RMS 1229) was discarded by it. The entry predates the capture gates (`stt_min_speech_ms`, the `no_speech` discard, the VAD warm-up), which now stop silence reaching Whisper at all |
...
| `stt_onset_frames` | `3` | Consecutive VAD-positive 30ms frames before a capture starts (90ms of continuous voice). Was effectively `1`: a door slam or a consonant from across the room started a capture, 750ms of silence ended it, the 250ms gate rejected it, and the loop re-entered — reopening the mic stream and discarding the first 210ms of whatever the user had begun saying. Live 2026-08-04: eight rejected 120ms captures in nine seconds. Set to `1` to restore the old behaviour |
| `stt_vad_warmup_frames` | `5` | Frames fed to the freshly built webrtcvad instance and ignored for onset. A fresh instance reports its first 3–5 frames as speech on quiet-room audio (reproduced offline 2026-09-08 on a device recording, all three aggressiveness modes), and every capture builds a fresh instance — so every capture opened with a fake 120–150ms utterance, the 250ms gate rejected it, and the loop re-entered: one rejected capture every 0.97s, ten in a row live. Level-independent, so quiet speech ("Okay." measured at RMS 81) is untouched; that is why `stt_min_speech_rms` stays off. `0` disables |
| `stt_speech_onset_timeout_s` | `6.0` | Seconds a capture waits for speech onset before returning `no_speech` (`0` = wait the full `max_record_seconds`). A silent window used to run the full 15s, so the idle tail of a session was 15–30s and 15s of background chatter reached Whisper. **Idle behaviour now:** the idle clock starts when Bender *finishes replying* (previously when the user finished speaking, so a long answer ate the whole `silence_timeout`), an empty window returns after ~6s, and the session ends on the first empty window that starts more than `silence_timeout` (8s) after the last reply — in practice the second empty window, ~12s |
