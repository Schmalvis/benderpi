...
- `watchdog_quiet_hours_start` / `watchdog_quiet_hours_end` (default `23` / `7`, local time, wraps midnight) — suppresses the HA push during this window (cooldown bookkeeping still happens). Does NOT special-case `session_liveness` — expect one morning notification if Bender wasn't used before bed; raise `max_hours_without_session` if that's noise rather than signal.
- `watchdog_notify_domain` / `watchdog_notify_service` (default `persistent_notification` / `create`, always exists on any HA instance) — the `POST /api/services/<domain>/<service>` target for alert pushes. Override with a real `notify.*` service (e.g. `notify` / `mobile_app_yourphone`) for a push notification instead of an HA persistent-notification card.
...
**Watchdog alert push (`bender-watchdog.timer`):** `scripts/watchdog.py` is a library, not a daemon — nothing ran `run_checks()` between deploys until now. `scripts/watchdog_notify.py` runs the checks, sweeps `logs/*.jsonl` older than `log_retention_days`, and pushes any non-empty alert list to Home Assistant's `POST /api/services/<domain>/<service>` (reusing `HA_URL`/`HA_TOKEN`), deduped/cooled-down via `logs/.watchdog_state.json` (gitignored) and suppressed during `watchdog_quiet_hours_*`. Install manually (git pull does not install units): `sudo cp systemd/bender-watchdog.service systemd/bender-watchdog.timer /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable --now bender-watchdog.timer`. Runs every 15 minutes (`OnUnitActiveSec=15min`). Manual run: `venv/bin/python scripts/watchdog_notify.py`. **The default `watchdog_notify_domain`/`_service` (`persistent_notification`/`create`) always works but only shows up in HA's UI, not as a push notification** — set them to a real `notify.*` service (e.g. your HA mobile app's) for that.
